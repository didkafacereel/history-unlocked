...
| **1.1 — first item** | **The day's video in the app** (user's call, 26 Sep): the nightly short shown at the top of its date as "today's film". Hosting on **Cloudflare R2** (user's choice: zero egress; a year of clips is under 1 GB). Already produced every night by `make.mjs` into `video/app-clips/` — 720p, ~2 MB, cut before the TikTok end card (its "subscribe to find out when the app comes out" makes no sense inside the app), faded out. Still to build: R2 bucket + upload step, a small `videos.json` published beside the manifest (never inside it — the manifest is 22 MB and must not be republished nightly), a feed card + `expo-video` player (a native module, so a new build, not an OTA), and a privacy-policy line for the new host. | Reuses an asset made every night anyway; a daily reason to open the app. Not before launch: it needs a new native module and review while the first review is pending, and clips exist only from 24 Sep — the closed-test weeks let them accumulate. |
